...
if ping -c 1 -W 2 192.168.0.27 > /dev/null 2>&1; then
    ...
    # Check CAN interface
    CAN_STATUS=$(ssh -i "$SSH_KEY" "$ROVER" "ip link show can1 2>/dev/null | grep 'state UP' || echo 'DOWN'" 2>/dev/null || echo "ERROR")
    if [[ "$CAN_STATUS" == *"UP"* ]]; then
        echo -e "${GREEN}✓ CAN Interface: UP${NC}"
    else
        echo -e "${RED}✗ CAN Interface: DOWN${NC}"
        echo "  Run: ssh -i $SSH_KEY $ROVER sudo ip link set can1 type can bitrate 1000000"
        echo "       ssh -i $SSH_KEY $ROVER sudo ip link set can1 up"
    fi
